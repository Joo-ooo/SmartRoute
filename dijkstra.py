...
# Get the hotel names from the Excel file
...
# Define timezone
# define Singapore timezone for date and time calculations
singapore = timezone('Asia/Singapore')

# Set the day type and vehicle type
day_type = "Weekdays"
vehicle_type = "Heavy Goods Vehicles/Small Buses"

# Load gantry locations from CSV
...
# Load ERP data from JSON
with open('data/test_erp.json', 'r') as file:
    data = json.load(file)


# Function to calculate the total time taken to traverse a path
...
# TSP
# Function to solve the Travelling Salesman Problem (TSP) for multiple destinations
def get_shortest_path(graph, destinations, returnToChangi, travelTime):
    ...
    filtered_permutations = [perm for perm in all_permutations if perm[-1] == changi_node] if returnToChangi else all_permutations
...
# Function to calculate time taken to traverse a route
...
# Function to calculate the total ERP cost based on zoneID and ERP time
...
# function to calculate total ERP cost based on zoneID and ERP time
